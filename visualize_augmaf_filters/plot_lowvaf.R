...
readmaf <- function(filepath,distype,deflist=c(1,2,3,5,6,7,9,10,14,16,17)){
  ...
  df$dbSNP_RS=ifelse(grepl('^rs',df$dbSNP_RS),df$dbSNP_RS,"novel")
  return(df)
}
...
brca_df=readmaf(brca,"BRCA");
df<- na.omit(rbind(hnsc_df,luad_df,lusc_df,brca_df))

#Unrestricted data
clean_table=df[(df$nvaf == 0),!names(df) %in% c("n_ref_count","n_alt_count","t_ref_count","t_alt_count")]

write.table(clean_table,file=paste(indir,"only_tumour_variants.txt",sep=""),append=FALSE,quote=FALSE,row.names=FALSE,sep="\t")
...
write.table(clean_matrix, file=paste(indir,"only_tumour_variants_mat.txt",sep=""), row.names=TRUE, col.names=TRUE,sep="\t")
...
#Get counts
library(dplyr);library(reshape2); library(gplots); library(RColorBrewer)
outdir=paste(indir,"/results/plots_4cancers/",sep="")
cohort_freq=0.05
pdf(paste(outdir,"gene_cohortfreq_",cohort_freq,"_vaf_",lowvaf,"_hist.pdf",sep=""),height=60,width=20)
gene_hmap = as.data.frame(count(unique(gene_slow[,c("disease","Hugo_Symbol","Matched_Norm_Sample_Barcode")]),disease,Hugo_Symbol))
summary_disease = aggregate(df$Matched_Norm_Sample_Barcode, by = df[c('disease')],function(x) length(unique(x)))
gene_hmap = merge(gene_hmap,summary_disease,by="disease",all.x=TRUE); gene_hmap$dfreq=gene_hmap$n/gene_hmap$x
gene_hmapd=acast(gene_hmap[gene_hmap$Hugo_Symbol %in% (gene_hmap[(gene_hmap$dfreq < cohort_freq)&(gene_hmap$n > 1),"Hugo_Symbol"]),],Hugo_Symbol~disease,value.var='n'); gene_hmapd[is.na(gene_hmapd)]<-0; 
heatmap.2(na.omit(gene_hmapd),cellnote=gene_hmapd, lwid=c(1.5,5),lhei=c(1.5,14),tracecol=NA,dendrogram="row",cexCol=1,cexRow=1,margins=c(4,16),
          main=paste("Pan cohort with cohort freq max ", cohort_freq, " \n max vaf ",lowvaf," genes, patient frequency across cohorts",sep=""), col=colorRampPalette(c("white","green","blue2","violet","purple"))(100), notecol="black")
gene_hmapd=acast(gene_hmap[gene_hmap$Hugo_Symbol %in% (gene_hmap[(gene_hmap$dfreq >= cohort_freq)&(gene_hmap$n > 1),"Hugo_Symbol"]),],Hugo_Symbol~disease,value.var='n'); gene_hmapd[is.na(gene_hmapd)]<-0; 
heatmap.2(na.omit(gene_hmapd),cellnote=gene_hmapd, lwid=c(1.5,5),lhei=c(1.5,14),tracecol=NA,dendrogram="row",cexCol=1,cexRow=1,margins=c(4,16),
          main=paste("Pan cohort with cohort freq atleast ", cohort_freq, " \n max vaf ",lowvaf," genes, patient frequency across cohorts",sep=""), col=colorRampPalette(c("white","green","blue2","violet","purple"))(100), notecol="black")
dev.off()
